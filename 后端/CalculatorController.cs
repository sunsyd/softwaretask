...
namespace CalculatorBackend
{
    ...
    public class CalculatorController : ControllerBase
    {
        ...
        {
            _logger.LogInformation("收到计算请求：Expression={Expression}", request.Expression);
            try
            {
                var result = SafeEvaluate(request.Expression);
                _logger.LogInformation("计算成功：表达式={Expression}，结果={Result}", request.Expression, result);
                return Ok(new { Result = result });
            }
            catch (ValidationException ex) // 自定义验证错误
            {
                _logger.LogWarning(ex, "输入验证失败：表达式={Expression}", request.Expression);
                return BadRequest(new
                {
                    errorType = "VALIDATION_ERROR",
                    message = ex.Message,
                    details = ex.Details // 可选扩展信息
                });
            }
            catch (MathOperationException ex) // 自定义数学错误
            {
                _logger.LogWarning(ex, "数学运算错误：表达式={Expression}", request.Expression);
                return BadRequest(new
                {
                    errorType = "MATH_ERROR",
                    message = ex.Message,
                    affectedFunction = ex.FunctionName // 错误关联的函数
                });
            }
            catch (EvaluationException ex) // NCalc解析错误
            {
                _logger.LogWarning(ex, "表达式解析错误：表达式={Expression}", request.Expression);
                return BadRequest(new
                {
            ...
            {
                _logger.LogError(ex, "服务器内部错误：表达式={Expression}", request.Expression);
